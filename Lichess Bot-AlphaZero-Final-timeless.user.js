...
const CONFIG = {
    ...
    // Time Management - AlphaZero is TIMELESS (no panic, ever)
    // Stays composed regardless of clock - plays with same precision
    lowTimeThreshold: 5000,      // Slight adjustment, NOT panic
...
function getThinkTime(phase, posType, timeLeft) {
    ...
    // Time management - AlphaZero stays composed
    if (timeLeft < CONFIG.lowTimeThreshold) speedMultiplier *= 0.7;
    if (timeLeft < 3000) speedMultiplier *= 0.6;
    if (timeLeft < 1500) speedMultiplier *= 0.5;
...
function getDepth(phase, posType, timeLeft) {
    ...
    // Time management - AlphaZero maintains depth, never panics
    if (timeLeft < CONFIG.lowTimeThreshold) depth = Math.max(11, depth - 1);
    if (timeLeft < 3000) depth = Math.max(10, depth - 2);
    if (timeLeft < 1500) depth = Math.max(9, depth - 3);
